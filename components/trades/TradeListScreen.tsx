              ...
              <table className="w-full min-w-[1020px] text-left">
                ...
                <tbody>{trades.map((trade) => (
                  <tr key={trade._id} tabIndex={0} onClick={() => setSelected(trade)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(trade); } }} className="cursor-pointer border-t border-white/5 text-sm transition-colors hover:bg-white/5 focus:bg-white/5 focus:outline-none">
                    ...
                    <td className="p-4">{mode === "active" ? <><span className="text-xs font-bold uppercase">{trade.status}</span>{scope === "copy" && trade.sourceTradeClosedAt && <p className="mt-1 max-w-48 text-xs text-amber-300">{trade.sourceTradeCloseMessage || "The pro trader has closed this trade. Please close yours manually."}</p>}</> : <><p className={`text-xs font-bold uppercase ${trade.tradeResult === "profit" ? "text-secondary" : trade.tradeResult === "loss" ? "text-tertiary" : "text-slate-400"}`}>{trade.tradeResult || trade.status}</p><p className="mt-1 font-mono text-xs">{trade.realizedPnl ?? "—"} USDT</p></>}</td>
