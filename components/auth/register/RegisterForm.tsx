...
import SponsoredTraderModal from "./SponsoredTraderModal";
...
export default function RegisterForm() {
  ...
  return (
    <>
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-10 xl:p-14 overflow-y-auto bg-[var(--color-surface-container-lowest)]">
        <div className="w-full max-w-sm py-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Sponsored trader option - only for Copy Trader */}
            {role === "CopyTrader" && (
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-[var(--color-on-surface-variant)] mb-1.5">
                  Program Selection
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <RiUserFollowLine
                        className={`text-base flex-shrink-0 ${!sponsored ? "text-[var(--color-secondary)]" : "text-[var(--color-on-surface-variant)]"}`}
                      />
                      <div>
                        <p className="text-xs font-bold text-[var(--color-on-surface)]">
                          Standard
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl text-left transition-all border relative overflow-hidden group ${
                      sponsored
                        ...
                        : "border-[var(--color-outline-variant)]/30 bg-[var(--color-surface-container-highest)] hover:border-[var(--color-tertiary)]/40"
                    } disabled:opacity-50`}
                  >
                    <div className="absolute -top-1 -right-1 opacity-20 group-hover:opacity-30 transition-opacity">
                      <RiStarLine className="text-2xl text-[var(--color-tertiary)]" />
                    </div>
                    <div className="flex items-center gap-2.5">
                      <RiStarLine
                        className={`text-base flex-shrink-0 ${sponsored ? "text-[var(--color-tertiary)]" : "text-[var(--color-on-surface-variant)]"}`}
                      />
                      <div>
                        <p className="text-xs font-bold text-[var(--color-on-surface)]">
                          Sponsored
      ...
      {/* Sponsored Trader Modal */}
      <SponsoredTraderModal
